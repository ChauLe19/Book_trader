    ...
    return (
        <div style={{ width: "90%", margin: "auto", display: "grid", gridTemplateColumns: "1fr 1fr", overflow: "hidden" }}>
            ...
            <div id="sellers" style={{ height: "100%", overflow: "scroll", border: "0.5rem solid #192A56", borderWidth: "0.25rem" }}>
                ...
                {forSaleBooks && forSaleBooks.length != 0 ? forSaleBooks.map(elem => <SellerBookBox key={elem.id} book_id={elem.book_id} sellerUsername={elem.owner_username} price={elem.price} dateForSale={elem.date_for_sale} />): <p>There is no one selling this book. Please try again at a different time</p>}
            </div>
        </div>
    );
}
